  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        ...
        console.error("Failed to load menus in summary", error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="flex flex-col min-h-screen bg-app-bg pb-32 overflow-x-hidden relative">
      ...
      <div className="flex-1 px-6 pb-12 pt-16 relative z-10 w-full max-w-sm mx-auto">
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-[11px] font-black tracking-[0.2em] text-[#B21B7A] uppercase mb-4 text-center">Tunjukkan QR ke Kasir</h2>
          
          <div className="bg-white py-8 px-6 rounded-[2rem] w-full text-center mb-6 relative overflow-hidden border border-[#FBCFE8] flex flex-col items-center">
            ...
            <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 z-10 mb-4 inline-block">
              <QRCodeSVG 
                value={code} 
                size={200}
                level="M"
            ...
            <p className="text-xs text-gray-400 font-semibold flex items-center justify-center space-x-1.5 opacity-80 relative z-10 mt-3 mix-blend-multiply">
          ...
          <div className="w-full text-left mb-4 mt-2">
            ...
            <div className="space-y-3 w-full">
              {isLoading ? (
                <div className="text-center text-sm text-gray-400 py-4">Memuat detail pesanan...</div>
...
export default function SummaryPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-app-bg text-[#B21B7A] font-bold">Memuat...</div>}>
      <SummaryContent />
    </Suspense>
  );
}